...
export async function fetchPlaceDetails(placeId: string, formatted: boolean, fieldMask?: string) {
  const res = await axios.get(
    `https://places.googleapis.com/v1/places/${placeId}`,
    {
      headers: {
        "Content-Type": "application/json",
        "X-Goog-Api-Key": process.env.GOOGLE_API_KEY,
        "X-Goog-FieldMask": fieldMask ? fieldMask :
...
router.get("/details/:placeId", async (req: Request, res: Response) => {
  ...
  const isFormatted = (Array.isArray(formatted) ? formatted[0] : formatted) === "true";

  if (!fieldMask) {
    return res.status(400).json({ error: "X-Goog-FieldMask header is required" });
  }

  try {
    const responseData = await fetchPlaceDetails(placeIdValue, isFormatted, fieldMask);
