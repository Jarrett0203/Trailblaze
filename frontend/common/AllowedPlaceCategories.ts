...
export function getCategories(types: string[]): PlaceTypeCategory[] {
  const typeSet = new Set(types);
  console.log(types);
  return (Object.keys(PLACE_TYPE_CATEGORIES) as PlaceTypeCategory[]).filter(
    (category) =>
      PLACE_TYPE_CATEGORIES[category].types.some((type) => typeSet.has(type)),
  );
}

export type PlaceTypeCategory = keyof typeof PLACE_TYPE_CATEGORIES;
export type PlaceType =
  (typeof PLACE_TYPE_CATEGORIES)[PlaceTypeCategory]["types"][number];
